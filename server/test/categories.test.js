...
describe('POST /dinas/categories [ERROR CASE]', () => {
    ...
    test('Expect Error when access_token is wrong', (done) => {
        ...
        request(app)
            ...
            .send(payload)
            .set('Accept', 'application/json')
            .set('access_token', 'access_tokenpalsu')
            .then((response) => {
                expect(response.status).toBe(401)
                expect(response.body.message).toContain('Invalid access token')
                done()
    ...
    test('Expect Error when property name is null', (done) => {
        let access_token = jwtSign({
            id: dinas._id,
            email: dinas.email,
        })
...
describe('PUT /dinas/categories/:id [ERROR CASE]', () => {
    test('Expect Error when user not login', (done) => {
        let payload = {
            name: 'Masalah Kriminal',
        }
    ...
    test('Expect Error when id is not found', (done) => {
        let access_token = jwtSign({
            id: dinas._id,
            email: dinas.email,
        })
        ...
        let id = '6145fa9b6aa09548b59af38e'
        request(app)
            .put(`/dinas/categories/${id}`)
            .send(nameNull)
            .set('Accept', 'application/json')
...
describe('DELETE /dinas/categories/:id [ERROR CASE]', () => {
    test('Expect Error when user not login', (done) => {
        let id = categories._id
        request(app)
            .delete(`/dinas/categories/${id}`)
