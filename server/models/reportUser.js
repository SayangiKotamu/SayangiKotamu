...
class ReportUser {
    ...
    static async findByCategory(category){
      try {
        let data = await getDatabase().collection("ReportUser").findOne({ "category": category })
        return data
      } catch (error) {
        return {
          name:"Not Found",
    ...
    static editReport(payload, id) {
      return getDatabase().collection("ReportUser").updateOne({
        _id: ObjectId(id)
      },{
        $set: payload,
      }
    ...
    static deleteReport(id) {
      return getDatabase().collection("ReportUser").deleteOne({ _id: ObjectId(id) });
    }
